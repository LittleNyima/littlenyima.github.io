...
> 举个例子作为直观的理解：如果一个模型有 10 层，那么其中某个 rank 的优化器只会存储其中 2 层所对应的梯度、动量、方差等信息，其余部分均被丢弃。在 `optimizer.step()` 时，也只有这两层的权重被更新。更新权重后，该 rank 上的模型就有两层新的权重以及八层旧的权重，最后通过 all-gather 获得所有的新权重。
...
与 ZeRO-1 同理，我们可以模仿对优化器状态分片的方式，同样对梯度也进行分片。然后在反向传播期间，不再对梯度执行 all-reduce，而是只执行 reduce-scatter 操作。这样，我们在内存中只需要存储所需梯度的 $1/N_d$，从而在 ZeRO-1 的基础上节约更多内存。

> 有的人可能要问了，如果只存储所需梯度的一部分，那丢弃部分的梯度岂不是无法得到正确结果了？在实现的时候，实际上梯度的 reduce-scatter 是随着反向传播同时进行的。也就是说，每当有一层的梯度被计算出来，就会同时触发一个 hook，在这个 hook 中 reduce-scatter 操作会在梯度被丢弃前同步到对应的节点上，从而保证虽然多余的梯度被丢弃，但保留部分的梯度依然正确。
